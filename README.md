...
> **Noch nie mit Claude Code gearbeitet?** Beginnen Sie auf [jaredrhod.com](https://jaredrhod.com): Wählen Sie Ihre Situation aus, und Sie werden auf den richtigen Weg geleitet.

**Läuft auf:** Entwickelt und getestet auf Claude Code; jede Terminal-KI, die Dateien lesen und schreiben kann, funktioniert – wenn auch mit kleinen Ecken und Kanten. Der 20-Dollar-Pro-Tarif reicht aus.
...
- **[CLAUDE.md](templates/CLAUDE.md):** die Startkonfiguration. Sie kommt in den Ordner, von dem aus du Claude Code ausführst (dein **Arbeitsverzeichnis**), und wird **außerhalb deines Tresors** aufbewahrt, damit der Tresor reine Notizen enthält und nicht durcheinandergerät, sobald du mehr als ein Projekt hast. Claude Code lädt sie bei jeder Sitzung automatisch und verweist die KI auf deinen Tresor. Enthält die Identität deines Agenten (seinen Namen, seine Rolle und seine Persönlichkeit) sowie deine Startsequenz und die Regeln, die nicht außer Kraft gesetzt werden dürfen. **Diese Datei ist bereits einsatzbereit:** Die Identität meines eigenen Agenten ist bereits eingetragen und deutlich als der einzige Abschnitt gekennzeichnet, den du austauschen kannst, falls du lieber deinen eigenen haben möchtest.
...
- **[DAILY-NOTE.md](templates/DAILY-NOTE.md):** die Vorlage für die tägliche Notiz. Wird **in Ihrem Vault** unter `01 – Daily Notes/Daily Note Template.md` abgelegt. Jede tägliche Notiz wird daraus erstellt, sodass das Protokoll ein einheitliches, übersichtliches Format behält.
- **[MEMORY.md](templates/MEMORY.md):** Der Zeiger auf den eigenen Speicher von Claude Code. Er wird im **Projektordner von Claude Code** (`~/.claude/projects/...`, nicht in Ihrem Vault) abgelegt. Er leitet den nativen Speicher zurück in den Vault um, sodass niemals zwei Speicherebenen entstehen, die voneinander abweichen.

## Aktualisierung

Das Build-Skript, die Vorlagen und der Assistent werden kontinuierlich verbessert. Wenn Sie eine Kopie dieses Repos auf Ihrer Festplatte gespeichert haben, geben Sie Ihrem Agent beispielsweise folgenden Befehl: **„Lade die neueste Version von ai-memory-vault herunter und teile mir mit, was sich geändert hat.“** Aktualisierungen betreffen ausschließlich die Dateien des Repos selbst. Dein Tresor, deine Notizen und deine CLAUDE.md gehören dir und befinden sich niemals in diesem Repo, sodass nichts, was du erstellt hast, überschrieben werden kann. Über fullstack-agent installiert? `./fullstack-agent/update.sh` aktualisiert alle Teile auf einmal und gibt aus, was sich geändert hat.
